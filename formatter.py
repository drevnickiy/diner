...
function setSelectedDriver(driverName) {{
    ...
    if (FIREBASE_DB_URL) {{
        try {{
            ...
            const url = `${{FIREBASE_DB_URL.replace(/\/$/, '')}}/driver/${{today}}.json`;
            fetch(url, {{
                method: 'PUT',
                headers: {{ 'Content-Type': 'application/json' }},
                body: JSON.stringify({{ name: driverName, updated_at: new Date().toISOString() }})
            }}).catch(e => {{}});
        }} catch(e) {{}}
    }}
}}

async function fetchDriverFromCloud() {{
    if (FIREBASE_DB_URL) {{
        try {{
            const today = getTodayStr();
            const url = `${{FIREBASE_DB_URL.replace(/\/$/, '')}}/driver/${{today}}.json`;
            ...
            if (res.ok) {{
                ...
                if (data && data.name) {{
                    ...
                    localStorage.setItem(getSelectedDriverKey(), selectedDriver);
                }}
            }}
        }} catch(e) {{}}
    }}
