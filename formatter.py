...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f1f5f9; margin: 0; padding: 24px 12px;">
    <div style="max-width: 1060px; margin: 0 auto;">
        ...
        <div id="voting-widget" style="background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0; padding: 24px; margin-bottom: 24px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.05); box-sizing: border-box;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; border-bottom: 2px solid #f1f5f9; padding-bottom: 16px; margin-bottom: 20px;">
                <div>
                    ...
                    <p style="margin: 4px 0 0 0; font-size: 13px; color: #64748b;">
                        Обирайте заклад, вводьте ім'я та голосуйте! Переможець визначається за кількістю голосів.
                    </p>
                </div>
                <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                    <button id="config-db-btn" onclick="configureFirebaseUrl()" style="background: #475569; color: #ffffff; border: none; padding: 8px 14px; border-radius: 8px; font-weight: 600; font-size: 13px; cursor: pointer; display: flex; align-items: center; gap: 6px; transition: all 0.2s;">
                        ☁️ Налаштувати БД
                    </button>
            ...
            <div id="winner-banner" style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); border: 2px solid #f59e0b; border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 24px; align-items: flex-start;">
                ...
                <div style="flex: 1 1 320px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 18px;">
                    <form id="vote-form" onsubmit="handleVoteSubmit(event)" style="display: flex; flex-direction: column; gap: 14px;">
                        
                        <div>
                            <label for="voter-name-input" style="display: block; font-size: 13px; font-weight: 700; color: #334155; margin-bottom: 6px;">
                                👤 Ваше ім'я <span style="color: #ef4444;">*</span>
                            </label>
                            <input type="text" id="voter-name-input" required placeholder="Наприклад: Богдан" style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; box-sizing: border-box; transition: border-color 0.2s;">
                        </div>
...
// Set your Firebase Realtime DB URL here or via settings
let FIREBASE_DB_URL = localStorage.getItem('diner_firebase_url') || '';
...
async function fetchVotes() {{
    ...
    if (FIREBASE_DB_URL) {{
        try {{
            const url = `${{FIREBASE_DB_URL.replace(/\/$/, '')}}/votes/${{today}}.json`;
            const res = await fetch(url);
            if (res.ok) {{
                const data = await res.json();
                const votesList = data ? Object.values(data) : [];
                currentVotes = votesList;
    ...
    // 2. Try Local Server API (http://localhost:5050/api/votes)
    try {{
...
function renderVotes(votes, summary) {{
    const goingVotes = votes.filter(v => v.choice === 'going');
    ...
    document.getElementById('count-total').innerText = votes.length;

    const total = votes.length || 1;
    const goingPct = (goingVotes.length / total) * 100;
    const notGoingPct = (notGoingVotes.length / total) * 100;

    document.getElementById('progress-going').style.width = (votes.length ? goingPct : 0) + '%';
    document.getElementById('progress-not-going').style.width = (votes.length ? notGoingPct : 0) + '%';
    ...
    if (winnerName && winnerVotes > 0) {{
        ...
        winnerTitleEl.innerText = 'Очікування перших голосів...';
    }}
    ...
    votes.forEach(v => {{
        ...
        const badgeBg = isGoing ? '#ecfdf5' : '#fff1f2';
        const badgeBorder = isGoing ? '#a7f3d0' : '#fecdd3';
        const badgeText = isGoing ? '#047857' : '#be123c';
        const icon = isGoing ? '🚗 Їдемо' : '🏠 Не їдемо';
        const restStr = (isGoing && v.restaurant) ? `<span style="background: rgba(255,255,255,0.8); border: 1px solid #cbd5e1; border-radius: 6px; padding: 2px 6px; font-size: 11px; color: #334155; margin-left: 4px;">📍 ${{escapeHtml(v.restaurant)}}</span>` : '';
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; background: ${{badgeBg}}; border: 1px solid ${{badgeBorder}}; border-radius: 8px; padding: 8px 12px;">
            ...
            <button onclick="deleteVote('${{escapeHtml(v.name)}}')" title="Видалити голос" style="background: transparent; border: none; color: #94a3b8; font-size: 14px; cursor: pointer; padding: 2px 6px; border-radius: 4px; transition: color 0.2s;" onmouseover="this.style.color='#ef4444'" onmouseout="this.style.color='#94a3b8'">✕</button>
        </div>
        `;
    }});
...
async function handleVoteSubmit(event) {{
    ...
    const choice = document.querySelector('input[name="vote-choice"]:checked').value;
    const restaurant = document.getElementById('voter-restaurant-select').value;
    const note = document.getElementById('voter-note-input').value.trim();

    if (!name) return;

    localStorage.setItem('diner_user_name', name);
    ...
    if (FIREBASE_DB_URL) {{
        try {{
            const url = `${{FIREBASE_DB_URL.replace(/\/$/, '')}}/votes/${{today}}/${{encodeURIComponent(name)}}.json`;
            const res = await fetch(url, {{
                method: 'PUT',
    ...
    try {{
        const res = await fetch('/api/votes', {{
            method: 'POST',
            headers: {{ 'Content-Type': 'application/json' }},
            body: JSON.stringify({{ name, choice, restaurant, note }})
        }});
        if (res.ok) {{
            const data = await res.json();
            if (data.success) {{
...
async function deleteVote(name) {{
    ...
    // 2. Try Local Server API
    try {{
        const res = await fetch(`/api/votes?name=${{encodeURIComponent(name)}}`, {{ method: 'DELETE' }});
        if (res.ok) {{
            const data = await res.json();
            if (data.success) {{
                currentVotes = data.votes;
...
function configureFirebaseUrl() {{
    const current = FIREBASE_DB_URL || '';
    const input = prompt('Введіть URL вашої бази даних Firebase Realtime DB:\n(наприклад: https://diner-votes-default-rtdb.firebaseio.com)', current);
    if (input !== null) {{
        FIREBASE_DB_URL = input.trim();
        localStorage.setItem('diner_firebase_url', FIREBASE_DB_URL);
        alert(FIREBASE_DB_URL ? '✅ URL хмарної бази даних збережено!' : 'ℹ️ Хмарну базу даних вимкнено.');
        fetchVotes();
    }}
}}
</script>
...
function copyVotesForTelegram() {{
    ...
    let text = `🍱 *Голосування на обід (${{todayStr}})*\\n\\n`;
    text += `🚗 *Їдемо (${{going.length}}):*\\n`;
    if (going.length > 0) {{
        going.forEach(v => {{
            const rStr = v.restaurant ? ` [${{v.restaurant}}]` : '';
            const note = v.note ? ` _(${{v.note}})_` : '';
            text += `  • ${{v.name}}${{rStr}}${{note}}\\n`;
        }});
    }} else {{
        text += `  _(нікого)_\\n`;
    }}

    text += `\\n🏠 *Не їдемо (${{notGoing.length}}):*\\n`;
