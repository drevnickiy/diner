...
async function deleteVote(name) {{
    ...
    // 3. Background Local Server Sync
    try {{
        fetch(`/api/votes?name=${{encodeURIComponent(name)}}`, {{ method: 'DELETE' }})
            .then(res => res.json())
            .then(data => {{
                if (data && data.success) {{
                    currentVotes = data.votes;
...
function configureFirebaseUrl() {{
    ...
    const input = prompt('Введіть URL вашої бази даних Firebase Realtime DB (наприклад: https://diner-votes-default-rtdb.firebaseio.com):', current);
    if (input !== null) {{
