...
async function handleVoteSubmit(event) {{
    ...
    const newVote = {{ name, choice, restaurant, note, updated_at: new Date().toISOString() }};
    if (idx >= 0) {{
        currentVotes[idx] = newVote;
    }} else {{
...
async function deleteVote(name) {{
    try {{
        const res = await fetch(`/api/votes?name=${{encodeURIComponent(name)}}`, {{
            method: 'DELETE'
        }});
        if (res.ok) {{
            const data = await res.json();
