...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f1f5f9; margin: 0; padding: 24px 12px;">
    <!-- Auth Modal Overlay -->
    <div id="auth-container" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(15,23,42,0.6); backdrop-filter: blur(4px); display: none; align-items: center; justify-content: center; z-index: 10000;">
        <div style="max-width: 400px; width: 90%; background: #ffffff; padding: 32px; border-radius: 16px; box-shadow: 0 20px 40px rgba(0,0,0,0.25); position: relative;">
            <button onclick="hideAuthModal()" style="position: absolute; top: 16px; right: 16px; background: none; border: none; font-size: 20px; color: #64748b; cursor: pointer;">✕</button>
    ...
    <div id="main-app" style="max-width: 1240px; margin: 0 auto; display: none;">
        ...
        <div style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); color: #ffffff; padding: 28px 24px; border-radius: 16px 16px 14px 14px; text-align: center; margin-bottom: 24px; box-shadow: 0 10px 25px -5px rgba(15,23,42,0.3);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                ...
                <div id="user-info-header" style="display: flex; align-items: center; gap: 8px;">
                    <!-- Filled by JS -->
                </div>
            </div>
    ...
    if (authToken) {{
        ...
        if (userHeader) {{
            userHeader.innerHTML = `
                <span style="font-size: 13px; font-weight: 700; background: rgba(255,255,255,0.2); padding: 4px 10px; border-radius: 12px;">👤 ${{escapeHtml(authUsername)}}</span>
                <button onclick="handleLogout()" style="background: rgba(239, 68, 68, 0.8); color: white; border: none; padding: 4px 12px; border-radius: 8px; font-size: 12px; font-weight: 700; cursor: pointer;">
                    Вийти 🚪
                </button>
            `;
        }}
        ...
        if (userHeader) {{
            userHeader.innerHTML = `
                <button onclick="showAuthModal()" style="background: #2563eb; color: white; border: none; padding: 6px 14px; border-radius: 8px; font-size: 13px; font-weight: 700; cursor: pointer;">
...
async function handleVoteSubmit(event) {{
    ...
    showModal("Голос успішно збережено!");
